...
class TagThroughForm(forms.ModelForm):
    class Meta:
        ...
        fields = ("tagobject",)
        widgets = {"tagobject": ModelSelect2Bootstrap(url="autocomplete-tags")}
...
class TeamForm(forms.ModelForm):
    ...
    class Meta:
        model = Team
        fields = ("story", "year", "disband_year", "year_prefix", "disband_year_prefix")
...
TeamSeasonFormSet = inlineformset_factory(
    Season, TeamSeason, form=TeamSeasonForm, extra=1
)
...
class PersonSeasonForm(forms.ModelForm):
    ...
    story = forms.CharField(
        label=_("Комментарии"), widget=forms.Textarea, required=False
    )
    ...
    class Meta:
        ...
        widgets = {
            "season": ModelSelect2Bootstrap(
                url="autocomplete-seasons", forward=["year"]
            ),
            "team": ModelSelect2Bootstrap(url="autocomplete-teams", forward=["year"]),
        }
...
PersonSeasonFormSet = inlineformset_factory(
    Season, PersonSeason, form=PersonRosterForm, extra=1
)


class ImportForm(forms.Form):
    input = forms.CharField(
        strip=False, widget=forms.Textarea(attrs={"cols": 80, "rows": 20})
    )
...
class ImportRosterForm(forms.Form):
    input = forms.CharField(
        strip=True, widget=forms.Textarea(attrs={"cols": 80, "rows": 20})
    )
