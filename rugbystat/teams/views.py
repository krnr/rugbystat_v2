...
class TeamAutocomplete(autocomplete.Select2QuerySetView):
    def get_queryset(self):
        ...
        if year:
            qs = qs.filter(
                Q(year__lte=year, disband_year__gte=year)
...
class TeamSeasonAutocomplete(autocomplete.Select2QuerySetView):
    def get_queryset(self):
        ...
        season = self.forwarded.get("season", None) or self.forwarded.get(
            "tourn_season", None
        )
        if season:
...
class TeamSeasonView(FormMixin, DetailView):
    ...
    def get_form_class(self):
        return (
            PersonSeasonInRosterForm
            if self.request.POST.get("person")
